...
class AssetPlanExtractorV2:
    ...
    def _debug_click(self, element, context: str = ""):
        ...
        return self.debug_manager.debug_click(element, context)
            ...
            """
            try:
                self.driver.execute_script(f"document.head.insertAdjacentHTML('beforeend', '{debug_css}');")
                self._show_debug_info("Debug mode enabled")
            except:
                pass
    
    def _highlight_element(self, element, highlight_type="highlight", duration=1.5):
        """Highlight an element for visual debugging with enhanced indicators."""
        if not self.debug_mode:
            return
            
        try:
            # Enhanced visual feedback for clicks
            if highlight_type == "clicked":
                # Add pulsing animation for clicks
                self.driver.execute_script("""
                    arguments[0].style.cssText += `
                        border: 4px solid #00ff00 !important;
                        background-color: rgba(0, 255, 0, 0.4) !important;
                        box-shadow: 0 0 20px #00ff00 !important;
                        animation: pulse-click 1s ease-in-out !important;
            ...
            # Scroll element into view (skip in extreme mode)
            if not self.extreme_mode:
                self.driver.execute_script("arguments[0].scrollIntoView({block: 'center', behavior: 'smooth'});", element)
            
            # Shorter wait for better flow
            import time
            time.sleep(duration)
            ...
            if highlight_type == "clicked":
                self.driver.execute_script(
                    "arguments[0].style.cssText = arguments[0].style.cssText.replace(/border[^;]*;|background-color[^;]*;|box-shadow[^;]*;|animation[^;]*;|z-index[^;]*;/g, '');", 
                    element
                )
            else:
                self.driver.execute_script(
    ...
    def _show_debug_info(self, message: str, duration: float = 3.0):
        ...
        try:
            ...
            setTimeout(function() {{
                var elem = document.getElementById('scraper-debug-info');
                if (elem) elem.remove();
            }}, {duration * 1000});
            """
            self.driver.execute_script(info_script)
